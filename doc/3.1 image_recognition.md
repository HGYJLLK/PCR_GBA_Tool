...
这会在 `logs/ui/` 目录下生成 `screenshot_YYYYMMDD_HHMMSS.png`（1280x720 分辨率），将其作为素材基础

![image-20260127215727986](./assets/image-20260127215727986.png)

使用 PS 处理
1. 在 Photoshop 中打开截图

![image-20260127220306079](./assets/image-20260127220306079.png)

2. 用矩形选框工具框选**按钮区域**（按住alt，在鼠标位置滚动滑轮可以放到那一块地方，方便选取）

![image-20260127220415725](./assets/image-20260127220415725.png)

3. 新建一个图层，然后把图层拉到下面

![image-20260127220555705](./assets/image-20260127220555705.png)

4. 选中新创建的图层，按快捷键 Alt+Delete（前景色填充，确保前景色是黑色）

![image-20260127225852798](./assets/image-20260127225852798.png)

5. 导出为PNG格式，保存到assets里的文件夹
...
**重要**：`assets/` 和 `module/` 的文件夹结构是**一一对应**的：

1. 在 `assets/` 下创建文件夹，如 `assets/daily/`
2. 放入处理好的图片
3. 运行 `python -m dev_tools.button_extract`
4. 自动生成 `module/daily/assets.py`
...
```python
# module/daily/assets.py
from module.base.button import Button

DAILY_BUTTON = Button(
    area=(600, 400, 680, 480),      # 自动计算的识别区域
    color=(100, 150, 255),           # 自动提取的平均颜色
    button=(600, 400, 680, 480),     # 点击区域（默认与 area 相同）
    file='./assets/daily/DAILY_BUTTON.png'
)
```
