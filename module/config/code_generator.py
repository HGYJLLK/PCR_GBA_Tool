...
class CodeGenerator:
    def __init__(self):
        self.tab_count = 0  # 记录当前缩进
        self.lines = []  # 记录配置文件代码行

    def generate(self) -> t.Iterable[str]:
        yield ""

    def add(self, line, comment=False, newline=True):
        # generator.tab_count = 1 时，add("hello") → 存下 "    hello\n"
    ...
    def Value(self, key=None, value=None, type_=None, **kwargs):
        # Value("name", "Alice") → 生成 name = 'Alice'
        # Value("name", "Alice", type_="str") → 生成 name: str = 'Alice'
        # Value("age", 30) → 生成 age = 30
        if key is not None:
    ...
    def Comment(self, text):
        # Comment("User settings") → 生成 # User settings
        for line in text.strip().split("\n"):
            line = line.strip()
            self.add(line, comment=True)

    def List(self, key=None):
        # 通过TabWrapper编写需要缩进的代码块
        if key is not None:
    ...
    def Object(self, object_class, key=None):
        if key is not None:
            ...
            return TabWrapper(
                self, prefix=f"{object_class}(", suffix=")", newline=False
            )
