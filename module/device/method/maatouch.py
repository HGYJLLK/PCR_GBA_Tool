...
class MaaTouch(Connection):
    ...
    def on_orientation_change_maatouch(self):
        ...
        logger.info(
            f"Orientation changed {self._maatouch_orientation} => {self.orientation}, re-init MaaTouch"
        )
        del_cached_property(self, "_maatouch_builder")
    ...
    def maatouch_init(self):
        ...
        stream = self.adb_shell(
            [
                f"CLASSPATH={self.config.MAATOUCH_FILEPATH_REMOTE}",
                "app_process",
                "/",
    ...
    def maatouch_send_sync(self, builder: MaatouchBuilder, mode=2):
        ...
        for n in range(3):
            ...
            if out == "Killed":
                raise MaaTouchNotInstalledError(
                    "MaaTouch died, probably because version incompatible"
                )
            if n == max_trial - 1:
    ...
    def maatouch_install(self):
        ...
        self.adb_push(
            self.config.MAATOUCH_FILEPATH_LOCAL, self.config.MAATOUCH_FILEPATH_REMOTE
        )
