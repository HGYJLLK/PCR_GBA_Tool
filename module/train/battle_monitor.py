"""
战斗监控模块 (Mixin)
职责：进战后开启"全set（立即发动）"、监控OCR倒计时、战斗结束后点击伤害报告
异步截图 → module/device/async_screenshot.py
异步 OCR  → module/ocr/async_ocr.py
时间解析  → module/ocr/ocr.py Duration.parse_time
"""
...
class BattleMonitor:
    """
    战斗监控 Mixin。
    宿主类需提供：self.device、self.appear()、self.match_template_color()
    """
    ...
    def enable_full_set(self, timeout=5.0):
        """
        进入战斗后，若"立即发动"处于 OFF 状态则点击开启（全set模式）。
    ...
    def monitor_until_end(self, use_droidcast=False, timeline=None):
        ...
        Args:
            use_droidcast (bool): True 使用 DroidCast，False 使用 NemuIpc（默认）
            timeline: Timeline 对象，定义时间轴点击动作（可选）

        Returns:
            bool: True 表示正常点击了 REPORT
        """
        logger.hr("Monitor battle timer", level=1)

        mode = "DroidCast" if use_droidcast else "NemuIpc"
        logger.info(f"截图后端: {mode}")
