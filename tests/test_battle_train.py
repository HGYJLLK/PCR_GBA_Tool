    ...
    def _monitor_battle_timer(self, ocr_engine, use_droidcast=False):
        ...
        try:
            while True:
                ...
                # ========== 解析 OCR 结果 ==========
                should_check_button = False  # 默认不检测按钮
                ...
                if text_result:
                    ...
                    if total_seconds is not None and 0 <= total_seconds <= 90:
                        ...
                        # 显示状态
                        status = f"\r{CLEAR_LINE}[{elapsed:>5.1f}s] 倒计时: {time_str} | 检测: {detection_count}"
...
def main():
    ...
    battle_train_test.start_battle_and_monitor(
        use_droidcast=args.droidcast, slow_mode=args.slow
    )
