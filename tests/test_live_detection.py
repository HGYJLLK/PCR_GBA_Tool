...
Templates are loaded from module/character/assets.py (TEMPLATE_{uid} objects),
which is generated by dev_tools/build_character_assets.py and covers all
★1/★3/★6 variants (735 templates total).
"""
...
def load_templates_gray():
    """Load all TEMPLATE_{uid} objects from module/character/assets.py.

    Display names are resolved from char_assets.UNIT_NAMES (uid → 中文名★级).
    Generated by dev_tools/build_character_assets.py (735 templates, ★1/★3/★6).
    """
    unit_names = getattr(char_assets, "UNIT_NAMES", {})
    entries = []
    for attr in sorted(dir(char_assets)):
        if not attr.startswith("TEMPLATE_"):
            continue
        obj = getattr(char_assets, attr)
        if not isinstance(obj, Template):
            continue
        uid = attr[len("TEMPLATE_"):]
        display_name = unit_names.get(uid, uid)
        try:
            gray = cv2.cvtColor(obj.image, cv2.COLOR_RGB2GRAY)
            entries.append((display_name, gray))
...
def run_live(device):
    ...
    logger.info(f"Loaded {len(templates_gray)} templates (thresh={threshold})")
